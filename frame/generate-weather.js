...
// Open-Meteo's "current" time comes back as a naive local timestamp
// (already adjusted for the `timezone` param, e.g. "2026-08-25T14:15") --
// format it directly rather than going through Date, which would
// re-interpret it in the server's own timezone (UTC on GitHub Actions).
function formatTime12h(isoLocalString) {
...
async function main() {
    ...
    if (now < daily.sunrise[0]) {
        astro = "Sunrise at " + formatTime12h(daily.sunrise[0]);
    } else if (now < daily.sunset[0]) {
        astro = "Sunset at " + formatTime12h(daily.sunset[0]);
    } else {
